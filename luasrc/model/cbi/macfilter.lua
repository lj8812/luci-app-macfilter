...
local function get_connected_clients()
    ...
    for line in dhcp_leases:gmatch("[^\r\n]+") do
        ...
        if #parts >= 5 then
            ...
            local clean_mac = sanitize_mac(mac)
            if clean_mac and validate_ipv4(ip) then
                hostname = (hostname ~= "*" and hostname ~= "") and hostname or nil
                if not devices[clean_mac] then
                    ...
                    devices[clean_mac].hostname = hostname or devices[clean_mac].hostname
                    devices[clean_mac].ip = ip  -- 更新为DHCP分配的IP
                    devices[clean_mac].active = devices[clean_mac].active or (os.time() - tonumber(ts) < 3600)
                end
            end
        end
    end
...
function btn.write(self, section)
    ...
    uci:foreach("macfilter", "rule", function(s)
        if s.mac then existing[sanitize_mac(s.mac) or ""] = true end
    end)
